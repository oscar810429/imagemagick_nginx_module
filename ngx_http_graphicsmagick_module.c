...
static char*	   ngx_http_graphicsmagick					 (ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static ngx_int_t   ngx_http_graphicsmagick_handler			 (ngx_http_request_t *r);
static ngx_str_t*  ngx_http_graphicsmagick_get_command		 (ngx_http_request_t *r);
static void		   ngx_http_graphicsmagick_command_handler	 (ngx_http_request_t *r);
static ngx_int_t   ngx_http_graphicsmagick_tokenize_command (ngx_http_request_t *r, ngx_str_t *cmd, ngx_array_t *a);
static u_char*     ngx_http_graphicsmagickd_map_uri_to_path (ngx_http_request_t *r, ngx_str_t *uri, ngx_str_t *path, size_t *root_length, size_t reserved);

static unsigned int ngx_http_graphicsmagick_convert	 (char **argv, size_t argc);

#define ngx_http_graphicsmagick_server_error(r) ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR)

static ngx_command_t  ngx_http_graphicsmagick_commands[] = {
	{ ngx_string("imagemagick"),
	  NGX_HTTP_LOC_CONF|NGX_CONF_NOARGS,
	  ngx_http_graphicsmagick,
...
static ngx_http_module_t  ngx_http_graphicsmagick_module_ctx = {
...
ngx_module_t  ngx_http_graphicsmagick_module = {
	NGX_MODULE_V1,
	&ngx_http_graphicsmagick_module_ctx, /* module context */
	ngx_http_graphicsmagick_commands,	/* module directives */
...
static char *
ngx_http_graphicsmagick(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_core_loc_conf_t  *clcf;

	clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
	clcf->handler = ngx_http_graphicsmagick_handler;
...
static ngx_str_t ngx_http_graphicsmagick_command_header = ngx_string("X-ImageMagick-Convert");
...
static ngx_int_t
ngx_http_graphicsmagick_tokenize_command(ngx_http_request_t *r, ngx_str_t *cmd, ngx_array_t *dst)
{
	u_char		 *pos;
	u_char		 *start_pos;
	u_char		 *data;
